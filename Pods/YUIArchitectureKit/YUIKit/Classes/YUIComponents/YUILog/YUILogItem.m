...
@implementation YUILogItem

+ (instancetype)logItemWithLevel:(YUILogLevel)level name:(NSString *)name logString:(NSString *)logString, ... {
    YUILogItem *logItem = [[self alloc] init];
    logItem.level = level;
    logItem.name = name;
    
    YUILogNameManager *logNameManager = [YUILogger sharedInstance].logNameManager;
...
- (NSString *)levelDisplayString {
    switch (self.level) {
        case YUILogLevelInfo:
            return @"YUILogLevelInfo";
        case YUILogLevelWarn:
            return @"YUILogLevelWarn";
        default:
            return @"YUILogLevelDefault";
    }
}
