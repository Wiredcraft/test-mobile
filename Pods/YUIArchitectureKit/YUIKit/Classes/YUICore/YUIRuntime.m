...
//  YUIKit
//
//  Created by YUI Team on 2018/9/5.
//
...
@implementation YUIPropertyDescriptor

+ (instancetype)descriptorWithProperty:(objc_property_t)property {
    YUIPropertyDescriptor *descriptor = [[self alloc] init];
    ...
    descriptor.type = [YUIPropertyDescriptor typeWithEncodeString:[NSString stringWithUTF8String:type]];
    if (type != NULL) {
        free(type);
    }
...
int yui_getProjectClassList(classref_t **classes) {
    size_t count = 0;
    if (!!classes) {
...
BOOL yui_exists_dyld_image(const char *target_image_name) {
    const uint32_t imageCount = _dyld_image_count();
    for (uint32_t i = 0; i < imageCount; i++) {
