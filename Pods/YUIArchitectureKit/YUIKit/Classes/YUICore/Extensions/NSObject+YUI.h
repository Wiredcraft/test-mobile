 ...
 */
- (nullable id)yui_performSelectorToSuperclass:(SEL)aSelector;
...
/**
 ...
 @link http://stackoverflow.com/questions/14635024/using-objc-msgsendsuper-to-invoke-a-class-method @/link
 */
- (nullable id)yui_performSelectorToSuperclass:(SEL)aSelector withObject:(nullable id)object;
 ...
 *  @code
 *  CGFloat alpha;
 *  [view yui_performSelector:@selector(alpha) withPrimitiveReturnValue:&alpha];
 *  @endcode
 */
- (void)yui_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue;
 ...
 *  [control yui_performSelector:@selector(addTarget:action:forControlEvents:) withArguments:&target, &action, &events, nil];
 *  @endcode
 */
- (nullable id)yui_performSelector:(SEL)selector withArguments:(nullable void *)firstArgument, ...;
 ...
 *  UIEvent *event = xxx;
 *  BOOL isInside;
 *  [view yui_performSelector:@selector(pointInside:withEvent:) withPrimitiveReturnValue:&isInside arguments:&point, &event, nil];
 *  @endcode
 */
- (void)yui_performSelector:(SEL)selector withPrimitiveReturnValue:(nullable void *)returnValue arguments:(nullable void *)firstArgument, ...;
...
/**
 ...
 @param block  用于遍历的 block
 */
+ (void)yui_enumrateIvarsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Ivar ivar, NSString *ivarDescription))block;

/**
 使用 block 遍历指定 class 的所有属性，不包含 superclasses 里定义的 property
 
 @param block 用于遍历的 block，如果要获取 property 的信息，推荐用 YUIPropertyDescriptor。
 */
 ...
 */
+ (void)yui_enumratePropertiesOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(objc_property_t property, NSString *propertyName))block;

/**
 使用 block 遍历当前实例的所有方法，不包含 superclasses 里定义的 method
 */
- (void)yui_enumrateInstanceMethodsUsingBlock:(void (^)(Method method, SEL selector))block;
...
/**
 ...
 @param block    用于遍历的 block
 */
+ (void)yui_enumrateInstanceMethodsOfClass:(Class)aClass includingInherited:(BOOL)includingInherited usingBlock:(void (^)(Method method, SEL selector))block;
 ...
 */
+ (void)yui_enumerateProtocolMethods:(Protocol *)protocol usingBlock:(void (^)(SEL selector))block;

@end

@interface NSObject (YUI_KeyValueCoding)
...
//
// @note YUI 提供2种方式兼容系统的 access prohibited 异常：
// 1. 通过将配置表的 IgnoreKVCAccessProhibited 置为 YES 来全局屏蔽系统的异常警告，代码中依然正常使用系统的 valueForKey:、setValue:forKey:，当开启后再遇到 access prohibited 异常时，将会用 YUIWarnLog 来提醒，不再中断 App 的运行，这是首选推荐方案。
// 2. 使用 yui_valueForKey:、yui_setValue:forKey: 代替系统的 valueForKey:、setValue:forKey:，适用于不希望全局屏蔽，只针对某个局部代码自己处理的场景。
//
// @link https://github.com/Tencent/YUI_iOS/issues/617
//
// @param key ivar 属性名，支持下划线或不带下划线
// @return key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
// */
//- (nullable id)yui_valueForKey:(NSString *)key;
//
///**
...
// @return key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
// */
//- (void)yui_setValue:(nullable id)value forKey:(NSString *)key;
...
@interface NSObject (YUI_DataBind)
...
/**
 ...
 [cell yui_bindObject:indexPath forKey:@"indexPath"];
 }
 
 - (void)didTapButton:(UIButton *)button {
 // 2）在这里取出被点击的 button 的 indexPath 对象
 NSIndexPath *indexPathTapped = [button yui_getBoundObjectForKey:@"indexPath"];
 }
 @endcode
 */
- (void)yui_bindObject:(nullable id)object forKey:(NSString *)key;

/**
 给对象绑定上另一个对象以供后续取出使用，但相比于 yui_bindObject:forKey:，该方法不会 strong 强引用传入的 object
 */
- (void)yui_bindObjectWeakly:(nullable id)object forKey:(NSString *)key;

/**
 取出之前使用 bind 方法绑定的对象
 */
- (nullable id)yui_getBoundObjectForKey:(NSString *)key;
...
@property(nonatomic, assign) BOOL yui_shouldIgnoreUIKVCAccessProhibited;
@end
