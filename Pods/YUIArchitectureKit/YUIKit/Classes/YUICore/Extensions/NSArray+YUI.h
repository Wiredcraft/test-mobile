...
@interface NSArray<ObjectType> (YUI)
...
/**
 ...
 @return 合并完的结果
 */
+ (instancetype)yui_arrayWithObjects:(ObjectType)object, ...;

/**
 *  将多维数组打平成一维数组再遍历所有子元素
 */
- (void)yui_enumerateNestedArrayWithBlock:(void (NS_NOESCAPE^)(id obj, BOOL *stop))block;
 ...
 */
- (NSArray<ObjectType> *)yui_filterWithBlock:(BOOL (NS_NOESCAPE^)(ObjectType item))block;

/**
*  转换数组元素，将每个 item 都经过 block 转换成一遍 返回转换后的新数组
*/
- (NSArray *)yui_mapWithBlock:(id (NS_NOESCAPE^)(ObjectType item))block;
